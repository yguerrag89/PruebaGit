"""Empaqueta la aplicación Windows V0.13 sin sesiones, secretos ni servidor."""
...
TARGET = ROOT / "release/v0.13/Ilubox_WMS_Windows_v0.13_Manual_Asistida.zip"
...
with zipfile.ZipFile(TARGET, "w", zipfile.ZIP_DEFLATED) as archive:
    for path in sorted(SOURCE.rglob("*")):
        if path.is_file() and include(path):
            archive.write(path, Path("Ilubox_WMS_Windows_v0.13") / path.relative_to(SOURCE))
